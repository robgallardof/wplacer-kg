...
const mainCamoufoxActions = $('mainCamoufoxActions');
...
const openWebshareModalMainBtn = $('openWebshareModalMainBtn');
const preparePythonEnvMainBtn = $('preparePythonEnvMainBtn');
...
const TRANSLATIONS = {
    ...
    es: {
        appSubtitle: 'Panel de automatización para wplace',
        camoufoxSetupTitle: 'Configuración de Camoufox',
        camoufoxSetupDesc: 'Prepara el entorno una vez y luego sincroniza proxies desde aquí.',
...
const changeTab = (tabName) => {
    ...
    // Keep navigation predictable and avoid landing at stale scroll positions.
...
const srgbNonlinearTransformInv = (c) =>
...
buildPaletteCaches();
const paletteSyncPromise = syncPalette();
...
const fetchCanvas = async (txVal, tyVal, pxVal, pyVal, width, height) => {
    ...
    for (let i = 0; i < t.length; i += 4) {
        // Skip transparent pixels from the template itself.
        if (t[i + 3] === 0) continue;
...
const processImageFile = (file, callback) => {
    ...
    reader.onload = (e) => {
        ...
        image.onload = async () => {
            // Prefer server palette IDs before decoding the uploaded image.
...
const openWebshareModalHandler = async () => {
...
const preparePythonEnvHandler = async () => {
    try {
